...
public class 基数排序的队列实现 {
	...
	public static void RadixSort(int[]arr){
		...
		//使用队列的话就不能使用二维数组了,用于临时存储数据的队列的数组   Myqueue[10]就指定了是0~9的10个队列，相当于存取数据的桶
		...
		for(int x=0,n=1;x<maxLength;x++,n*=10){
			...
			for(int j=0;j<arr.length;j++){
				...
				//将当前遍历的数据放入指定的队列中
				temp[yushu].addIn(arr[j]);
				
				/*把当前遍历的数据放入指定的数组中  二维数组方式的存取值
			...
			//记录从桶中取的元素需要放的位置
			int index=0;
			//把队列中的所有数字从0~9的队列中取出来，
			for(int q=0;q<temp.length;q++){
				while(!temp[q].havaData()) {//调用Myqueue中的havedata方法看队列中是否还有数据，取出来，直到队列中没有数据 {
				      arr[index]=temp[q].poll();//挨个从队列中取出数据
				      index++;
